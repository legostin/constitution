...
|----------|------|-------------|
| `.constitution.yaml` | **Source of truth** | User / setup wizard |
...
4. Drift detection on `SessionStart` (default) + `validate --check-drift` CLI
5. Wire `type: plugin` into rule engine (infrastructure exists, just needs registry integration)
6. Single source of truth principle: document and enforce in setup/validate

### Phase 2: Rule Classes + Bypass (backward-compatible)
7. Add `class` field to Rule type (optional, inferred from severity if absent)
8. Emergency bypass: `constitution bypass` CLI + `.constitution-bypass.json` + audit logging
9. Update setup wizard to separate constraint/guidance/verification/observation
10. Update documentation and README trust model language

### Phase 3: State Machine (new feature)
11. `workflow` section in config
12. `.constitution-state.json` session state
13. Phase-aware guidance injection at UserPromptSubmit
14. Phase transitions: deterministic (cmd_check) / explicit (CLI) / heuristic (CEL)
15. `constitution phase` CLI: `next`, `set <id>`, `status`
16. Update orchestration patterns to use state machine

### Phase 4: Testing + Extensibility
17. Scenario-based behavioral test framework (`constitution test --scenarios`)
18. Scenario runner: multi-step YAML, state carryover, assertion engine
19. Plugin package format and discovery
20. Plugin registry (future: marketplace concept)
